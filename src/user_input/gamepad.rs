use bevy::{prelude::*, input::gamepad::GamepadEventRaw};
...
impl Plugin for GamepadExample {
    fn build(&self, app: &mut App) {
        ...
        .add_system_set(
            SystemSet::on_update(InputExample::GamepadAxis)
            .with_system(gamepad_axis)
        );
    }
}
...
fn gamepad_button_axis(
    input: Res<Axis<GamepadButton>>,
) {
    for button in 
...
fn gamepad_event_raw(
    mut events: EventReader<GamepadEventRaw>
){
    for e in events.iter() {
        println!("Raw Event: {:?}", e);
    }
...
fn list_gamepads(
    ...
    *delta += time.delta_seconds();
    ...
    println!("there are {} controlers connected\n they are:", gamepads.len());
