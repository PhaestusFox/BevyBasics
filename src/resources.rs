...
impl FromWorld for DefaultFont {
    fn from_world(world: &mut World) -> Self {
        DefaultFont(world.get_resource::<AssetServer>().unwrap().load("./Raleway-Bold.ttf"))
    }
}
...
fn add_new_custom(
    ...
    if new_custom.is_none() {return;}
    for color in new_custom.unwrap().0.iter() {
        color_weel.0.push(matts.add(StandardMaterial { base_color: *color, ..Default::default() }));
    }
