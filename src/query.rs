...
fn query_init(
    ...
    commands.spawn().insert(Speed(0.)).insert(BBDirection(Vec3::ZERO)).insert(Name::new("Bob"));
}

fn single(
    _query: Query<&SomeComponent>
){

}

fn multiple(
    _query: Query<(&SomeComponent, &SomeOtherComponent)>
){

}

fn query(
    query: Query<&Name, Or<(With<Speed>, With<Transform>)>>
){
...
fn no_conflic(
    _query: Query<&Name, Or<(With<Speed>, With<Transform>)>>
){

}

fn conflic(
    mut _query: Query<&mut Name, Or<(With<Speed>, With<Transform>)>>
){
...
fn entity_ref(
    _query: Query<(Entity, &SomeComponent)>
){
...
fn check_colision(player_tran: &Transform, player_size: &Size, obj_tran: &Transform, obj_size: &Size) -> bool {
...
fn move_player(player_tran: &mut Transform, player_size: &Size, obj_tran: &Transform, obj_size: &Size) {
    let player_center = player_tran.translation;
    let player_half_size = player_size.0 * 0.5;
    let obj_center = obj_tran.translation;
    let obj_half_size = obj_size.0 * 0.5;
