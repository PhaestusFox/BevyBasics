...
fn my_system(mut commands: Commands) {
    println!(
        "spawned entity {:?}",
        commands.spawn(MyCompontentUnit).id()
    );
    let entity = commands
        .spawn(MyCompontentWithName {
    ...
    println!(
        "spawned entity {:?}",
        commands
            .spawn(MyCompontentWithData("Hello".to_string()))
            .id()
    );
    println!(
        "spawned entity {:?}",
        commands
            .spawn(MyCompontentWithData("World".to_string()))
            .id()
    );
...
fn my_logic(
    ...
    if input.just_pressed(KeyCode::T) {
        println!(
            "the target is {:?}:{}",
            target.0,
            targets.get(target.0).unwrap().name
        );
    }
}

fn my_other_logic(input: Res<Input<KeyCode>>, unit: Query<&MyCompontentUnit>) {
    if input.just_pressed(KeyCode::P) {
        for _ in unit.iter() {
            println!("Unit");
        }
