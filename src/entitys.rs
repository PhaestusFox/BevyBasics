...
fn inside_world_check(
    ...
    for (ball, transform) in homing_balls.iter() {
        if transform.translation.x < world_size.0.x
            && transform.translation.x > -world_size.0.x
            && transform.translation.y < world_size.0.y
            && transform.translation.y > -world_size.0.y
            && transform.translation.z < world_size.0.z
            && transform.translation.z > -world_size.0.z
        {
            commands.entity(ball).remove::<HomingBall>().insert(BoxBall);
        }
    }
}
...
fn change_color(
    ...
    if input.just_pressed(KeyCode::R) {
        ...
        set_color = Some(get_color_index(
            &colors,
            ColorSet::Blue,
            input.get_pressed(),
        ));
...
fn spawn_ball_ui(mut commands: Commands, font: Res<DefaultFont>) {
...
fn ball_resource_init(mut commands: Commands, mut mesh: ResMut<Assets<Mesh>>) {
    ...
    prism.set_indices(Some(bevy::render::mesh::Indices::U32(vec![
        0, 1, 2, 3, 0, 1,
    ])));
...
fn get_color_index<'a>(
    wheel: &ColorWheel,
    set: ColorSet,
    input: impl ExactSizeIterator<Item = &'a KeyCode>,
